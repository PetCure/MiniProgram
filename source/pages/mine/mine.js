...
Page({
  ...
  loadHelps() {
    var that = this;
    helpApi.mine({ openid: app.globalData.userInfo.openid}, function (data) {
      console.log(data);
      if (data.length > 0) {
  ...
  onPullDownRefresh: function () {
    ...
    var that = this;
    helpApi.mine({ "new": this.latestpulltime, openid: app.globalData.userInfo.openid }, function (data) {
      console.log(data);
      if (data.length > 0) {
        that.latestpulltime = data[0].published_date;
        for (var i = 0; i < data.length; i++) {
  ...
  onReachBottom: function () {
    ...
    var that = this;
    helpApi.mine({ latest: this.latestreachtime, openid: app.globalData.userInfo.openid }, function (data) {
      console.log(data);
      if (data.length > 0) {
        that.latestreachtime = data[data.length - 1].published_date;
        var helps = that.data.helps;
